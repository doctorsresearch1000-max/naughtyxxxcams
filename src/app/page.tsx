...
export default function HomePage() {
  ...
  return (
    <main className="relative mx-auto flex h-[100dvh] w-full max-w-md select-none flex-col overflow-hidden bg-black text-white">
      ...
      <div
        ref={feedRef}
        className="relative h-full w-full snap-y snap-mandatory overflow-y-auto scroll-smooth pb-16 [-webkit-overflow-scrolling:touch]"
      >
        <div className="relative h-full w-full snap-start snap-always">
          ...
          {isMuted && (
            <button
              type="button"
              onClick={toggleSound}
              className="absolute left-1/2 top-1/2 z-30 -translate-x-1/2 -translate-y-1/2 cursor-pointer transition-transform active:scale-95"
            >
          ...
          {/* Touch overlay: captura swipe y primer toque sin pasar clics al iframe */}
          <div
            className={`absolute inset-0 z-[28] touch-pan-y ${
              isMuted ? "bg-transparent" : "pointer-events-none"
            }`}
            aria-hidden={!isMuted}
